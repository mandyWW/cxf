...
public class LocalTransportFactoryTest extends Assert {
    ...
    private void testInvocation(boolean isDirectDispatch) throws Exception {
        ...
        LocalJndiTransportFactory factory = new LocalJndiTransportFactory();
        
        EndpointInfo ei = new EndpointInfo(null, "http://schemas.xmlsoap.org/soap/http");
        ei.setAddress("http://localhost/test");

        LocalJndiDestination d = (LocalJndiDestination) factory.getDestination(ei, bus);
        ...
        if (isDirectDispatch) {
            m.put(LocalJndiConduit.DIRECT_DISPATCH, Boolean.TRUE);
        }    
        m.setDestination(d);
    ...
    class TestMessageObserver implements MessageObserver {
        ...
        public synchronized void onMessage(Message message) {
            try {
                message.remove(LocalJndiConduit.DIRECT_DISPATCH);
